...
/**
 * Annotated by VibeXie in 2015.4.19
 ...
 * 
 * openfire采用Blowfish对密码进行加密与解密
 * 对于加密，调用new BlowfishUtil("密钥").encryptString("明文密码");返回密文密码。
 * 对于解密，到用new BlowfishUtil("密钥").decryptString("密文密码");返回明文密码。
 * 
 * 如何进行实现的，可以不用去了解，只去使用即可，获取源文件下载openfire源码包，在org.jivesoftware.util包中。
 */
public class BlowfishUtil implements Encryptor {
	/**
	 * Annotated by VibeXie in 2015.4.19
	 */
